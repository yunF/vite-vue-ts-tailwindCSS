...
const defaultHeader: AxiosRequestConfig = {
  ...
  headers: {
    ...
    BindUrl: `${import.meta.env.VITE_APP_PROXY_TARGET}/${import.meta.env.VITE_APP_FE_PATH}/${import.meta.env.VITE_APP_TENANT_ID}`,
    /* 临时 state */
...
http.interceptors.request.use(
  (config: AxiosRequestConfig): any => {
    ...
    if (Cookie.getCookie(Constants.cookieKey.token)) {
      config.headers = {
        ...config.headers,
        Authorization: 'Bearer ' + Cookie.getCookie(Constants.cookieKey.token)
      }
    }
