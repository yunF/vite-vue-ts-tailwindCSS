...
api.userApi.loginUser().then((res) => {
  console.log(res)
})
</script>

<template>
  <div class="container relative grid flex-col items-center justify-center h-screen h-800 lg:max-w-none lg:grid-cols-2 lg:px-0">
    <a href="/examples/authentication" :class="cn(buttonVariants({ variant: 'ghost' }), 'absolute right-4 top-4 md:right-8 md:top-8')"> Login <i-iconoir-home-simple-door /> </a>
    <div class="relative flex-col hidden h-full p-10 text-white bg-muted dark:border-r lg:flex">
      <div class="absolute inset-0 bg-zinc-900" />
      <div class="relative z-20 flex items-center text-lg font-medium">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" class="w-6 h-6 mr-2">
